...
def create_admin_access_token(request: Request, data: dict, expire_second: int = None):
    ...
    to_encode = {
        k: base64.b64encode(str(v).encode("utf-8")).decode("utf-8") + "0"
...
def create_employee_access_token(data: dict, expires_delta: timedelta = None):
    ...
    to_encode = data.copy()
    expire = datetime.now() + (
        expires_delta or timedelta(minutes=EMPLOYEE_ACCESS_TOKEN_EXPIRE_MINUTES)
    )
...
async def validate_admin(request: Request, access_token_admin: str = Cookie(None)):
    ...
    try:
        ...
        token = {
            k: base64.b64decode(v[:-1].encode("utf-8")).decode("utf-8")
