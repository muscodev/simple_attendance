...
from ..sa.auth import create_admin_access_token, verify_password
from ..sa.db import AsyncSession, get_session
...
async def admin_login(
    ...
    try:
        ...
        if (
            ...
            or not verify_password(credential.password, admin.password_hash)
        ):
            raise HTTPException(
                status_code=400, detail="Invalid credentials or inactive"
            )
    ...
    token = create_admin_access_token(
        request, payload, settings.admin_access_token_expiry_minute * 60
    )
    response.set_cookie(
        "access_token_admin",
        token,
...
async def deactivate_tenant_employee(
    id: uuid.UUID,
    admin: User = Depends(get_admin),
    db: AsyncSession = Depends(get_session),
):
...
async def get_tenant_employees(
    admin: User = Depends(get_admin), db: AsyncSession = Depends(get_session)
):
    return await employee_repo.get_all(db, admin.tenant_id)


@router.get("/admin/tenant/employees/status")
async def get_tenant_employees_status(
    admin: User = Depends(get_admin), db: AsyncSession = Depends(get_session)
):
...
async def get_employee_attendance(
    ...
    data = await employee_service.get_attendance_by_date(
        admin.tenant_id, id, start_date, end_date, db
    )
    return data
...
async def create_tenant_geomarking(
    geolocation: GeoMarkingCreateschema,
    admin: User = Depends(get_admin),
    db: AsyncSession = Depends(get_session),
):
    geolocation_create = GeoMarkingCreate.model_validate(
        {**geolocation.model_dump(), "tenant_id": admin.tenant_id}
    )
    return await geomarking_repo.create(db, geolocation_create)


@router.get("/admin/tenant/geomarking/{id}")
async def get_tenant_geomarking(
    id: uuid.UUID,
    admin: User = Depends(get_admin),
    db: AsyncSession = Depends(get_session),
):
...
async def get_tenant_geomarkings(
    admin: User = Depends(get_admin), db: AsyncSession = Depends(get_session)
):
...
async def update_tenant_geomarking(
    id: uuid.UUID,
    update_location: GeoMarkingUpdate,
    admin: User = Depends(get_admin),
    db: AsyncSession = Depends(get_session),
):
